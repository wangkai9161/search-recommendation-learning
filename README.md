...
FM、DeepFM 结果使用 300 用户、2 Epoch；负采样主对照使用 1,000 用户、2 Epoch；Decoder-only 使用 100 用户、32 维、1 Epoch，不能与上表直接横向比较。
...
初步观察：Batch 内负采样与随机负采样的 Top-K 命中接近，但候选覆盖差异明显；Router 改进后，多兴趣模型的候选塌缩得到缓解。FM、DeepFM 已统一到 300 用户、2 Epoch；生成式模型因 CPU Transformer 评估成本较高，使用 100 用户、32 维、1 Epoch 的教学配置，不能与上表直接横向比较。Loss 数值不能跨 Loss 定义直接比较，当前结果仍需要多随机种子验证。
